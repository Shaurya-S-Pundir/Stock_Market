...
async def lifespan(app: FastAPI):
	...
	with Session(engine) as session:
		if not session.exec(select(Campaign)).first():
			session.add_all([
				Campaign(name="Ronnie", due_date = datetime.now()),
				Campaign(name="DK", due_date = datetime.now())
			])
			session.commit()
	yield
...
@app.get("/campaigns",response_model = API_Response[list[Campaign]])
async def campaign(session: SessionDep):
	data = session.exec(select(Campaign)).all()
	return {"data": data}

 
@app.get("/campaigns/{id}", response_model= API_Response[Campaign])
...
@app.post("/campaign", status_code = 201, response_model = API_Response[Campaign])
...
@app.put("/campaigns/{id}", response_model = API_Response[Campaign])
